...
| `BRAUNY_SANDBOX_IMAGE` | `braunycode-sandbox:1` | Image der Sandbox. Der Installer baut es aus `deploy/sandbox.Dockerfile` mit pytest, hypothesis und ruff, damit der Container ohne Netz prüfen und testen kann. Scheitert der Bau, wird `python:3.11-slim` benutzt — dann sind Testläufe als Beleg nicht möglich. Selbst gesetzt: der Installer baut nichts und nimmt den angegebenen Wert. |
